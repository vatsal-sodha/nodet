...
def cnn_model_fn(features, labels, mode):
  ...
  # Dense Layer
  pool5_flat = tf.reshape(pool4, [-1, 4 * 4 * 64])
...
def load_data(j):
  if j==0:
    file_list = glob('Train_Data/Train_Data/*')
    train_data = np.zeros((2198,4096))
  if j==1:
    file_list = glob('Test_Data/Test_Data/*')
...
def load_labels(j):
  if j==0:
    df = pd.read_csv('Train_Lables.csv')
  if j==1:
    df = pd.read_csv('Test_Lables.csv')
...
def main(unused_argv):
  ...
  nodet_classifier = tf.estimator.Estimator(model_fn=cnn_model_fn, model_dir="/tmp/nodet_convnet4_model")
  ...
  # Classifier
  nodet_classifier.train(input_fn=train_input_fn,steps=10000,hooks=[logging_hook])
