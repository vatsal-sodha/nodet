...
def load_sampled_data_(j):
  ...
  if j==0:
    file_list = glob('../data/OSDataSets/OSTrain_0.8_Neg_0.5_Data/*')
    train_data = np.zeros((trainExamples,4096))
  if j==1:
    file_list = glob('../../../test_subset6/*')
...
def savePreds(filename,model):
  ...
  if not os.path.exists(directory_path):
    ...
    os.makedirs(directory_path+'/tps')
    os.makedirs(directory_path+'/tns')
  ...
  for i in range(len(tns)):
    shutil.copy(tns[i],directory_path+'/tns')

  for i in range(len(tps)):
    shutil.copy(tps[i],directory_path+'/tps')
...
def main(unused_argv):
  ...
  nodet_classifier = tf.estimator.Estimator(model_fn=cnn_model_fn, model_dir="./../../../Models2/cnn_model4_OS_PosNeg50:50_TrainTest80:20")
