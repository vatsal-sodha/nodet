...
directory_train='../data/ObjdetDataSet/train_preProcessed_2/'
if not os.path.exists(directory_train):
	os.makedirs(directory_train)

for i in df_train['filename']:
	im = Image.open('../data/Lungs_without_resizing/'+i)
	copyfile('../data/Lungs_without_resizing/'+i, directory_train + i)

directory_test='../data/ObjdetDataSet/test_preProcessed_2/'
if not os.path.exists(directory_test):
	os.makedirs(directory_test)

for i in df_test['filename']:
	im = Image.open('../data/Lungs_without_resizing/'+i)
	copyfile('../data/Lungs_without_resizing/'+i, directory_test + i)

df_train.to_csv('../data/ObjdetDataSet/train_preProcessed_2.csv',index=False)
df_test.to_csv('../data/ObjdetDataSet/test_preProcessed_2.csv'	,index=False)