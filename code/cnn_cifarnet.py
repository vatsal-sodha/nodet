...
files.extractall('Train_Data')
files.close()
files = zf.ZipFile("Test_Data.zip", 'r')
files.extractall('Test_Data')
files.close()
...
def cnn_model_fn(features, labels, mode):
  ...
  conv1 = tf.layers.conv2d(inputs=input_layer,filters=3,kernel_size=[3, 3],padding='SAME',activation=tf.nn.relu)
  ...
  conv3 = tf.layers.conv2d(inputs=pool2,filters=32,kernel_size=[5, 5],activation=tf.nn.relu)

  pool3 = tf.layers.max_pooling2d(inputs=conv3, pool_size=[2, 2], strides=2)

  conv4 = tf.layers.conv2d(inputs=pool3,filters=64,kernel_size=[5, 5],activation=tf.nn.relu)
  
  pool_flat = tf.reshape(conv4, [-1, 1 * 1 * 64])

  dense2 = tf.layers.dense(inputs=pool_flat, units = 64 , activation=tf.nn.relu)

  dense = tf.layers.dense(inputs=dense2, units = 64 , activation=tf.nn.relu)
